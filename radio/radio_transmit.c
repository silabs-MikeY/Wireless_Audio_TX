...
{
    if ((radio_tx_packet_buffer[packet_buffer_head].waiting_to_be_sent == true) && (radio_tx_packet_buffer[packet_buffer_head].sent == false))
    {
        // OVERFLOW
        printf("OVERFLOW\n");
        // assert(0);
    ...
    // printf("Filled Buffer at Index : %u\n", packet_buffer_head);

    radio_tx_packet_buffer[packet_buffer_head].micros_timestamp = scheduler__get_microsecond_ticks();
...
static void radio_transmit__mark_packet_as_successfully_sent(uint32_t index_sent)
{
    ...
    {
        // printf("Marking buffer: %u as sent\n", index_sent);
        if (radio_tx_packet_buffer[index_sent].sent == true)
        {
...
uint32_t radio_transmit__get_oldest_packet_ready_to_send(void)
{
    ...
    {
        if ((radio_tx_packet_buffer[i].waiting_to_be_sent == true) && (radio_tx_packet_buffer[i].sent == false))
        {
            if (radio_tx_packet_buffer[i].micros_timestamp < oldest_timestamp)
            {
                oldest_timestamp_index = i;
                oldest_timestamp = radio_tx_packet_buffer[i].micros_timestamp;
            }
        }
    }
...
static bool radio_transmit__send_packet(uint32_t buffer_index, bool retry)
{
    ...
    debug__increment_counter(number_of_TX_attempts);
    ...
    {
        radio_tx_packet_buffer[buffer_index].sent = true;
        ...
        if (retry)
        {
            debug__increment_counter(number_of_TX_retry_attempt_success);
        }
        else
        {
            debug__increment_counter(number_of_TX_attempt_success);
        }
    ...
    {
        debug__increment_counter(number_of_TX_attempt_failed);
        //      printf("Time: %u - ", (unsigned int)get_millisecond_ticks());
...
bool radio__process_event_tx(RAIL_Handle_t rail_handle, RAIL_Events_t events)
{
    ...
    if (events & RAIL_EVENT_TX_PACKET_SENT)
    {
        ...
        if (retry)
        {
            debug__increment_counter(number_of_TX_retry_success);
        }
        else
        {
            debug__increment_counter(number_of_TX_success);
        }
    ...
    else
    {
        if (events & RAIL_EVENTS_TX_COMPLETION)
        {
            ...
            if (retry)
            {
                debug__increment_counter(number_of_TX_retry_attempt_failed);
            }
            else
            {
                debug__increment_counter(number_of_TX_attempt_failed);
            ...
            if (events & RAIL_EVENT_TX_ABORTED)
            {
                ...
                if (retry)
                {
                    debug__increment_counter(number_of_TX_retry_fail_abort);
                }
                else
                {
                    debug__increment_counter(number_of_TX_fail_abort);
                }
            }
            else if (events & RAIL_EVENT_TX_BLOCKED)
            {
                ...
                if (retry)
                {
                    debug__increment_counter(number_of_TX_retry_fail_block);
                }
                else
                {
                    debug__increment_counter(number_of_TX_fail_block);
                }
            }
            else if (events & RAIL_EVENT_TX_UNDERFLOW)
            {
                ...
                if (retry)
                {
                    debug__increment_counter(number_of_TX_retry_fail_underflow);
                }
                else
                {
                    debug__increment_counter(number_of_TX_fail_underflow);
                }
            }
            else if (events & RAIL_EVENT_TX_SCHEDULED_TX_MISSED)
            {
                ...
                if (retry)
                {
                    debug__increment_counter(number_of_TX_retry_fail_missed);
                }
                else
                {
                    debug__increment_counter(number_of_TX_fail_missed);
                }
            }
            else if (events & RAIL_EVENT_TX_CHANNEL_BUSY)
            {
                ...
                if (retry)
                {
                    debug__increment_counter(number_of_TX_retry_fail_busy);
                }
                else
                {
                    debug__increment_counter(number_of_TX_fail_busy);
                }
            }
            else
            {
