...
void sl_rail_util_init(void)
{
  ...
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "RAIL_AddStateBuffer3 failed, return value: 0x%04" PRIX32,
             status);
#if   (INIT_INSTANCES > 3)
  status = RAIL_AddStateBuffer4(RAIL_EFR32_HANDLE);
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "RAIL_AddStateBuffer4 failed, return value: 0x%04" PRIX32,
             status);
#endif
#if   (INIT_INSTANCES > 4)
  for (int i = 0; i < (INIT_INSTANCES - 4); i++) {
    ...
    APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
               "RAIL_AddStateBuffer(%d) failed, return value: 0x%04" PRIX32,
