...
typedef struct new_counters_s {
  ...
  char counter_string_buffer[COUNTER_NAME_MAX_LENGTH + 16];
...
int32_t counters__register_counter(const char *input_counter_name,
  ...
  new_counters[counters_used_count].counter_string_buffer_value_start_index =
      sprintf(new_counters[counters_used_count].counter_string_buffer,
              " %u : %s,", (unsigned int)counters_used_count,
              input_counter_name);
#endif
...
void counters__save_and_print_counters(uint32_t current_timestamp) {
  ...
  counters__printf(true, "Counters saved at timestamp: %u",
...
#if (USE_STATIC_STRING_BUFFERS_FOR_COUNTER_PRINTING == 1)
      sprintf(new_counters[printing_counter_index].counter_string_buffer +
                  new_counters[printing_counter_index]
                      .counter_string_buffer_value_start_index,
              " %u\n",
              (unsigned int)new_counters[printing_counter_index]
                  .saved_counter_value);
      counters__printf_static_string(
          false,
          new_counters[printing_counter_index].counter_string_buffer);
#else
      counters__printf(
          false, " %u : %s, %u\n", printing_counter_index,
          new_counters[printing_counter_index].counter_name,
          new_counters[printing_counter_index].saved_counter_value);
#endif
      printing_counter_index++;
    } else {
      counters__printf(false, "\n");
